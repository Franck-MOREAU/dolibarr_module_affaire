...
<tr <?php echo 'id="row-'.$line->id.'" '.$bcdd[$var]; ?>>
	...
	<?php } ?>
	<td class="linecoldescription minwidth300imp"><?php $coldisplay++; ?><div
			id="line_<?php echo $line->id; ?>"></div>
	<?php
	if (($line->info_bits & 2) == 2) {
		?>
		<a
		href="<?php echo DOL_URL_ROOT.'/comm/remx.php?id='.$this->socid; ?>">
		<?php
		$txt = '';
	...
	<td class="linecoledit" align="center"><?php $coldisplay++; ?>
		...
		<?php } else { ?>
		<a
		href="<?php echo $_SERVER["PHP_SELF"].'?id='.$this->id.'&amp;action=editline&amp;lineid='.$line->id.'#line_'.$line->id; ?>">
		<?php echo img_edit(); ?>
		</a>
		<?php } ?>
	</td>
	...
	<td align="center" class="linecolmove tdlineupdown"><?php $coldisplay++; ?>
		<?php if ($i > 0) { ?>
		<a class="lineupdown"
		href="<?php echo $_SERVER["PHP_SELF"].'?id='.$this->id.'&amp;action=up&amp;rowid='.$line->id; ?>">
		<?php echo img_up('default',0,'imgupforline'); ?>
		</a>
		<?php } ?>
		<?php if ($i < $num-1) { ?>
		<a class="lineupdown"
		href="<?php echo $_SERVER["PHP_SELF"].'?id='.$this->id.'&amp;action=down&amp;rowid='.$line->id; ?>">
    ...
    <?php } else { ?>
    <td align="center"
		<?php echo ((empty($conf->browser->phone) && empty($disablemove)) ?' class="linecolmove tdlineupdown"':' class="linecolmove"'); ?>><?php $coldisplay++; ?></td>
	<?php } ?>
<?php } else { ?>
...
<?php
	...
	if (! empty($extrafieldsline)) {
		print $line->showOptionals($extrafieldsline, 'view', array(
				'style' => $bcdd[$var],
				'colspan' => $coldisplay
		), '', '', empty($conf->global->MAIN_EXTRAFIELDS_IN_ONE_TD) ? 0 : 1);
	}
}
?>
<?php if ($object->element == 'commande') {?>
...
<tr <?php echo 'id="row-'.$line->id.'" '.$bcdd[$var]; ?>>
	<td class="linecoldescription" style="border-bottom-style: none"
		colspan="2"><?php $coldisplay++; ?><div
			id="line_<?php echo $line->id; ?>"></div>
	<?php
	if (($line->info_bits & 2) == 2) {
		?>
	...
	<td align="right" class="linecolqty nowrap"
		style="border-bottom-style: none"><?php $coldisplay++; ?>
	<?php
	...
	<td align="right" class="linecoluht nowrap"
		style="border-bottom-style: none"><?php $coldisplay++; ?><?php echo price($line->subprice); ?></td>

	<td align="right" class="linecolmargin1 nowrap margininfos"
		style="border-bottom-style: none"><?php $coldisplay++; ?><?php echo price($line->pa_ht); ?></td>

	<td align="right" class="linecoluht nowrap"
		style="border-bottom-style: none"><?php $coldisplay++; ?><?php echo price($line->array_options["options_buyingprice_real"]); ?></td>

	<?php if(empty($line->array_options["options_fk_supplier"])){?>
	<td align="right" class="liencolht nowrap"
		style="border-bottom-style: none"><?php $coldisplay++; ?><?php echo price($line->total_ht-($line->qty * $line->pa_ht)); ?></td>
	<?php }else{?>
	<td align="right" class="liencolht nowrap"
		style="border-bottom-style: none"><?php $coldisplay++; ?><?php echo price($line->total_ht-$line->array_options["options_buyingprice_real"]); ?></td>
	<?php }?>
	<?php
	$soltrs = array(
			'GOLD',
    ...
    <?php } else { ?>
    <td align="center"
		<?php echo ((empty($conf->browser->phone) && empty($disablemove)) ?' class="linecolmove tdlineupdown"':' class="linecolmove"'); ?>
		style="border-bottom-style: none"><?php $coldisplay++; ?></td>
	<?php } ?>
...
<tr <?php echo 'id="row-'.$line->id.'" '.$bcdd[$var]; ?>>
	<td colspan="10" style="border-style: none"><b><span
			style="text-decoration: underline;">Commentaire:</span></b> <?php echo $line->desc;?>
</td>
</tr>
<?php }?>
