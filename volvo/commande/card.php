...
// Action Recalc
if ($action == 'recalc') {
	$object->updatevhpriceandvnc ( GETPOST ( 'prixtot', 'int' ) );
	$action = '';
...
if ($action == 'updateline' && $user->rights->commande->creer && GETPOST ( 'save' )) {
	...
	if (! $error) {
		$result = $object->updateline ( GETPOST ( 'lineid' ), $description, $pu_ht, GETPOST ( 'qty' ), GETPOST ( 'remise_percent' ), $vat_rate, $localtax1_rate, $localtax2_rate, 'HT', $info_bits, $date_start, $date_end, isset ( $_POST ["type"] ) ? $_POST ["type"] : $line->product_type, GETPOST ( 'fk_parent_line' ), 0, $fournprice, $buyingprice, $label, $special_code, $array_options, GETPOST ( 'units' ) );
		if ($result >= 0) {
			if (empty ( $conf->global->MAIN_DISABLE_PDF_AUTOUPDATE )) {
				// Define output language
				$outputlangs = $langs;
...
if ($action == 'create' && $user->rights->commande->creer) {
	...
	if ($object->id > 0) {
		...
		if ($action == 'shipped' && empty ( $ok )) {
			$form = new Form ( $db );
			$formconfirm = $form->formconfirm ( $_SERVER ["PHP_SELF"] . '?id=' . $object->id, $langs->trans ( 'CloseOrder' ), $langs->trans ( 'ConfirmCloseOrder' ), 'confirm_shipped', array (
					array (
							'type' => 'date',
							'name' => 'date_cloture',
		...
		if ($action == 'classifybilled' && empty ( $ok )) {
			$form = new Form ( $db );
			$formconfirm = $form->formconfirm ( $_SERVER ["PHP_SELF"] . '?id=' . $object->id, $langs->trans ( 'BilledOrder' ), $langs->trans ( 'ConfirmBilledOrder' ), 'confirm_billed', array (
					array (
							'type' => 'date',
							'name' => 'date_billed',
		...
		if ($action == 'setpayed' && empty ( $ok )) {
			$form = new Form ( $db );
			$formconfirm = $form->formconfirm ( $_SERVER ["PHP_SELF"] . '?id=' . $object->id, $langs->trans ( 'ClassifyPayed' ), $langs->trans ( 'ConfirmPayedOrder' ), 'confirm_payed', array (
					array (
							'type' => 'date',
							'name' => 'date_payed',
		...
		if ($action == 'update_pv') {
			$form = new Form ( $db );
			$formconfirm = $form->formconfirm ( $_SERVER ["PHP_SELF"] . '?id=' . $object->id, 'Modifier prix de vente', '', 'recalc', array (
					array (
							'type' => 'text',
							'name' => 'prixtot',
		...
		print '</td>';
		if ($action != 'editconditions' && $object->statut == 0)
			print '<td align="center"><a href="' . $_SERVER ["PHP_SELF"] . '?action=editconditions&amp;id=' . $object->id . '">' . img_edit ( $langs->trans ( 'SetConditions' ), 1 ) . '</a></td>';
		print '</tr></table>';
		print '</td>';
		...
		print '</td>';
		if ($action != 'editdemandreason' && $object->statut == 0)
			print '<td align="center"><a href="' . $_SERVER ["PHP_SELF"] . '?action=editdemandreason&amp;id=' . $object->id . '">' . img_edit ( $langs->trans ( 'SetDemandReason' ), 1 ) . '</a></td>';
		print '</tr></table>';
		print '</td>';
		...
		if ($action != 'presend' && $action != 'editline') {
			...
			if (empty ( $reshook )) {
				...
				if ($object->statut == Commande::STATUS_DRAFT && $user->rights->affaires->volvo->update_cost) {
				...
				if ($object->statut == Commande::STATUS_DRAFT && $object->total_ttc >= 0 && $numlines > 0 && ($ok == 1 || $ok < 0) && ((empty ( $conf->global->MAIN_USE_ADVANCED_PERMS ) && ! empty ( $user->rights->commande->creer )) || (! empty ( $conf->global->MAIN_USE_ADVANCED_PERMS ) && ! empty ( $user->rights->commande->order_advance->validate )))) {
